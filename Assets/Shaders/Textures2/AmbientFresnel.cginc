...
sampler2D NormalMap;
sampler2D HeightMap;
sampler2D DiffuseIBL;
sampler2D SpecularIBL;
...
float4 frag (v2f i) : SV_Target
{
    ...
    #ifdef IS_BASE_PASS
        float3 diffuseIBL = tex2Dlod(DiffuseIBL, float4(DirectionToRect(n), 0, 0)).xyz;
        diffuse += diffuseIBL;
    #endif
    ...
    specular *= _LightColor0.xyz;
        ...
        float mip = (1 - Gloss) * 6;
        float3 specularIBL = tex2Dlod(SpecularIBL, float4(DirectionToRect(viewReflection), mip, mip)).xyz;
        specular += specularIBL * SpecularIntensity + fresnel;
    #endif
